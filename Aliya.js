...
function getConfigPath(baseName, ext = ".json") {
	try {
		...
		if (fs.existsSync(devPath)) {
			console.log(`⚡ [ALIYA-CORE] » Successfully initialized ${baseName}.dev${ext}`);
			return devPath;
		} else if (fs.existsSync(normalPath)) {
			console.log(`⚡ [ALIYA-CORE] » Successfully initialized ${baseName}${ext}`);
			return normalPath;
		} else {
			throw new Error(`🚫 [SYSTEM-FAILURE] » Configuration missing: [${baseName}${ext} | ${baseName}.dev${ext}]`);
		}
	} catch (err) {
...
global.GoatBot = global.AliyaBot;
...
const watchAndReloadConfig = (dir, type, prop, logName) => {
	...
	fs.watch(dir, (eventType) => {
		if (eventType === type) {
			const oldConfig = global.AliyaBot[prop];
			setTimeout(() => {
				try {
					if (isFirstModified) {
					...
					global.AliyaBot[prop] = JSON.parse(fs.readFileSync(dir, 'utf-8'));
					log.success(logName, `Reloaded ${dir.replace(process.cwd(), "")}`);
				}
				catch (err) {
					log.warn(logName, `Can't reload ${dir.replace(process.cwd(), "")}`);
					global.AliyaBot[prop] = oldConfig;
				}
				finally {
...
global.AliyaBot.envGlobal = global.AliyaBot.configCommands.envGlobal;
global.AliyaBot.envCommands = global.AliyaBot.configCommands.envCommands;
global.AliyaBot.envEvents = global.AliyaBot.configCommands.envEvents;
